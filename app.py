...
# Two separate ChromaDB clients - one for each collection
council_chroma_client = PersistentClient(
    path=os.getenv('CHROMA_DB_DIR', '/models/chroma_db'),
    settings=Settings(anonymized_telemetry=False)
)

articles_chroma_client = PersistentClient(
    path=os.getenv('ARTICLES_CHROMA_DB_DIR', './chroma_db'),  # Your local directory
    settings=Settings(anonymized_telemetry=False)
)
...
class DualRAG(dspy.Module):
    ...
    def forward(self, question, start_date, end_date, n_results=5, source_type="both"):
        ...
        n_results: total number of results to return across all sources
        """
...
# Load both collections (from different ChromaDB instances)
try:
    council_collection = council_chroma_client.get_collection(
        name="city_council",
        embedding_function=embed_fn
    )
    print(f"Loaded city_council collection from {os.getenv('CHROMA_DB_DIR', '/models/chroma_db')}")
    print(f"   ({council_collection.count()} documents)")
except Exception as e:
    print(f" City council collection not found: {e}")
    council_collection = None

try:
    articles_collection = articles_chroma_client.get_collection(
        name="nola_articles",
        embedding_function=embed_fn
    )
    print(f"Loaded nola_articles collection from {os.getenv('ARTICLES_CHROMA_DB_DIR', './chroma_db')}")
    print(f"   ({articles_collection.count()} articles)")
except Exception as e:
    print(f" Articles collection not found: {e}")
    print(f"   Make sure you've run article_vectorize.py first!")
...
<body>
  <h1>Ask a question about New Orleans City Governance</h1>
  <form method="post">
